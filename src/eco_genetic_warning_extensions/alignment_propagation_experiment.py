...
def paired_risk_interval(aligned_loss_anti_no: int, aligned_no_anti_loss: int, n_pairs: int, *, z: float = 1.96) -> dict[str, float]:
    if n_pairs <= 0:
        raise ValueError("n_pairs must be positive")
    b = int(aligned_no_anti_loss)
    c = int(aligned_loss_anti_no)
    diff = (b - c) / n_pairs
    variance_d = (b + c) / n_pairs - diff * diff
    variance_d = max(0.0, variance_d)
    se = math.sqrt(variance_d / n_pairs)
    return {"risk_difference_anti_minus_aligned": diff, "standard_error": se, "ci95_lower": diff - z * se, "ci95_upper": diff + z * se}
...
def approximate_power(n_pairs: int, *, absolute_risk_difference: float = 0.05, discordance_probability: float = 0.412, z_alpha: float = 1.96) -> float:
...
def _dynamics_parameters(condition: str, seed: int, generations: int):
    ...
    return DynamicsParameters(
        patch_areas=(1.0, 1.0, 1.0, 1.0), generations=generations,
        initial_population=(40, 40, 40, 40), initial_interaction=(0.65, 0.75, 0.85, 0.95),
        initial_high_allele_frequency=bundle, initial_trait_abundance=trait_abundance_rows(condition),
        density_capacity=40.0, area_reference=1.0, interaction_feedback=4.5,
        interaction_barrier=0.50, trait_grid_size=31, trait_occupancy_mode="finite_trait_bin_recruitment",
        genotype_trait_recruitment="two_kernel_recruitment", inheritance_weight=0.5,
        q_feedback_alpha=alpha, q_feedback_beta_trait=beta_trait, q_feedback_gamma_allele=gamma,
        migration_rate=0.0, random_seed=seed,
    )
...
def _run_one(condition: str, master_seed: int, replicate: int, max_horizon: int) -> dict[str, Any]:
    ...
    result = simulate_with_symmetric_allele_mutation(parameters, mutation_rate=0.0, interaction_barrier_schedule=barrier_schedule(max_horizon))
    ...
    return {"condition": condition, "master_seed": int(master_seed), "replicate": int(replicate), "trajectory_seed": int(seed), "trait_loss_time_post_baseline": loss_time}
...
def _paired_cell(attempts: Iterable[dict[str, Any]], *, horizon: int, replicates_per_seed: int, master_seeds: tuple[int, ...]) -> dict[str, Any]:
    selected = [row for row in attempts if int(row["master_seed"]) in master_seeds and int(row["replicate"]) < replicates_per_seed]
    by_key = {(str(row["condition"]), int(row["master_seed"]), int(row["replicate"])): row for row in selected}
    a_only = b_only = same_loss = same_no_loss = aligned_loss_count = anti_loss_count = 0
    for seed in master_seeds:
        for replicate in range(replicates_per_seed):
            ...
            aligned_loss_count += int(a_loss); anti_loss_count += int(b_loss)
            if a_loss and not b_loss: a_only += 1
            elif b_loss and not a_loss: b_only += 1
            elif a_loss and b_loss: same_loss += 1
            else: same_no_loss += 1
    n_pairs = len(master_seeds) * replicates_per_seed
    interval = paired_risk_interval(a_only, b_only, n_pairs)
    return {
        "horizon": int(horizon), "replicates_per_seed": int(replicates_per_seed), "n_pairs": int(n_pairs),
        "aligned_loss_count": int(aligned_loss_count), "anti_aligned_loss_count": int(anti_loss_count),
        "aligned_loss_rate": aligned_loss_count / n_pairs, "anti_aligned_loss_rate": anti_loss_count / n_pairs,
        "aligned_loss_anti_no_loss": int(a_only), "aligned_no_loss_anti_loss": int(b_only),
        "same_loss": int(same_loss), "same_no_loss": int(same_no_loss),
        "discordance_rate": (a_only + b_only) / n_pairs,
        "mcnemar_exact_p": _two_sided_binomial_p(a_only, b_only), **interval,
    }


def summarise_attempts(attempts: list[dict[str, Any]], protocol: dict[str, Any] | None = None) -> dict[str, Any]:
    protocol = load_protocol() if protocol is None else protocol
    contract = contract_from_protocol(protocol)
    cells = [_paired_cell(attempts, horizon=horizon, replicates_per_seed=reps, master_seeds=contract.master_seeds) for horizon in contract.horizons for reps in contract.nested_replicates_per_seed]
    ...
    return {
        "experiment_id": protocol["experiment_id"], "status": "completed_post_phase_v_propagation_experiment",
        "legacy_phase_v_unchanged": True, "horizons": list(contract.horizons),
        "nested_total_pairs": [len(contract.master_seeds) * r for r in contract.nested_replicates_per_seed],
        "primary_pair_count": primary_n,
        "planning_power": {str(n): approximate_power(n) for n in [len(contract.master_seeds) * r for r in contract.nested_replicates_per_seed]},
        "cells": cells, "primary_horizon_cells": primary_cells, "interpretation_rule": protocol["decision_language"],
    }
...
def run_experiment(protocol: dict[str, Any] | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    return summarise_attempts(attempts, protocol), attempts


def write_experiment(summary_path: str | Path, attempts_path: str | Path) -> tuple[Path, Path]:
    summary, attempts = run_experiment()
    summary_dest = Path(summary_path); attempts_dest = Path(attempts_path)
    summary_dest.parent.mkdir(parents=True, exist_ok=True); attempts_dest.parent.mkdir(parents=True, exist_ok=True)
