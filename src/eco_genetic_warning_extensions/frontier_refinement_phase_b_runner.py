"""Warning-blind Phase-B recurrent-transition frontier-refinement runner.

Phase B uses one predeclared ecological/deterioration anchor and four interior
p_star values between the historical rapid-loss and persistence endpoints. It
records source/baseline/trait-loss information only.
"""
...
def phase_b_cell(cell_index: int):
    cells = phase_b_cells()
...
def run_phase_b_cell(upstream_checkout: str | Path, cell_index: int) -> dict[str, Any]:
    checkout = Path(upstream_checkout)
    if not checkout.exists():
        raise FileNotFoundError(f"upstream checkout does not exist: {checkout}")

    phase_cell = phase_b_cell(cell_index)
    ...
    with _upstream_import_path(checkout):
        ...
        schedule = calibration.RampHoldSchedule(
            phase_cell.ramp_generations,
            phase_cell.hold_generations,
            anchor.normalised_barrier_increase,
        )

        for master_seed in PHASE_B_MASTER_SEEDS:
            spec = replace(
                experiments.standard_profile(),
                experiment_id=f"frontier_refinement_phase_b_{phase_cell.cell_index:02d}",
                generations=1,
                replicates=PHASE_B_REPLICATES_PER_SEED,
            ...
            with patched_protocol002_mutation_runner(mutation, coordinate):
                ...
                if len(cells) != 1:
                    raise RuntimeError("Phase-B refinement must return exactly one H1 cell")
                cell = cells[0]
...
def _build_artifact(phase_cell, attempts: list[dict[str, Any]]) -> dict[str, Any]:
    expected = len(PHASE_B_MASTER_SEEDS) * PHASE_B_REPLICATES_PER_SEED
    if len(attempts) != expected:
        raise RuntimeError(f"Phase-B cell must retain {expected} attempts")
    ...
    seed_blocks = []
    rates = []
    for seed in PHASE_B_MASTER_SEEDS:
    ...
    regime = "incomplete"
    if len(rates) == len(PHASE_B_MASTER_SEEDS):
        regime = classify_seed_rates(tuple(rates))

    return {
        "stage": "warning-blind recurrent-transition frontier refinement Phase B",
        ...
        "design": {
            ...
            "historical_bracket_batches": [619, 673],
        },
        "status_counts": {
...
def write_phase_b_cell(upstream_checkout: str | Path, cell_index: int, output: str | Path) -> Path:
    destination = Path(output)
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        json.dumps(run_phase_b_cell(upstream_checkout, cell_index), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return destination
