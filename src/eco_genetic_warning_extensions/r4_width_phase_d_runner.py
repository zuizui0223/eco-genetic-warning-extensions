"""High-rep warning-blind Phase-D R4-width runner."""
...
def phase_d_cell(cell_index: int):
    cells = phase_d_cells()
...
def run_phase_d_cell(upstream_checkout: str | Path, cell_index: int) -> dict[str, Any]:
    checkout = Path(upstream_checkout)
    if not checkout.exists():
        raise FileNotFoundError(f"upstream checkout does not exist: {checkout}")

    phase_cell = phase_d_cell(cell_index)
    ...
    with _upstream_import_path(checkout):
        ...
        for master_seed in PHASE_D_MASTER_SEEDS:
            spec = replace(
                experiments.standard_profile(),
                experiment_id=f"r4_width_phase_d_{phase_cell.cell_index:02d}",
                generations=1,
                replicates=PHASE_D_REPLICATES_PER_SEED,
            ...
            with patched_protocol002_mutation_runner(mutation, coordinate):
                ...
                if len(cells) != 1:
                    raise RuntimeError("Phase-D refinement must return exactly one H1 cell")
                cell = cells[0]
...
def _build_artifact(phase_cell, attempts: list[dict[str, Any]]) -> dict[str, Any]:
    expected = len(PHASE_D_MASTER_SEEDS) * PHASE_D_REPLICATES_PER_SEED
    if len(attempts) != expected:
        raise RuntimeError(f"Phase-D cell must retain {expected} attempts")
    ...
    rates = []
    sufficient = True
    for seed in PHASE_D_MASTER_SEEDS:
        ...
        if len(seed_eligible) < PHASE_D_MIN_BASELINE_ELIGIBLE_PER_SEED:
            sufficient = False
    ...
    if not sufficient or len(rates) != len(PHASE_D_MASTER_SEEDS):
    ...
    return {
        "stage": "warning-blind R4 width refinement Phase D",
...
def write_phase_d_cell(upstream_checkout: str | Path, cell_index: int, output: str | Path) -> Path:
    destination = Path(output)
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        json.dumps(run_phase_d_cell(upstream_checkout, cell_index), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return destination
