"""High-rep warning-blind Phase-C frontier reproducibility runner."""
...
def phase_c_cell(cell_index: int):
    cells = phase_c_cells()
...
def run_phase_c_cell(upstream_checkout: str | Path, cell_index: int) -> dict[str, Any]:
    checkout = Path(upstream_checkout)
    if not checkout.exists():
        raise FileNotFoundError(f"upstream checkout does not exist: {checkout}")

    phase_cell = phase_c_cell(cell_index)
    ...
    with _upstream_import_path(checkout):
        ...
        for master_seed in PHASE_C_MASTER_SEEDS:
            spec = replace(
                experiments.standard_profile(),
                experiment_id=f"frontier_reproducibility_phase_c_{phase_cell.cell_index:02d}",
                generations=1,
                replicates=PHASE_C_REPLICATES_PER_SEED,
            ...
            with patched_protocol002_mutation_runner(mutation, coordinate):
                ...
                if len(cells) != 1:
                    raise RuntimeError("Phase-C audit must return exactly one H1 cell")
                cell = cells[0]
...
def _build_artifact(phase_cell, attempts: list[dict[str, Any]]) -> dict[str, Any]:
    expected = len(PHASE_C_MASTER_SEEDS) * PHASE_C_REPLICATES_PER_SEED
    if len(attempts) != expected:
        raise RuntimeError(f"Phase-C cell must retain {expected} attempts")
    ...
    rates = []
    sufficient = True
    for seed in PHASE_C_MASTER_SEEDS:
        ...
        if len(seed_eligible) < PHASE_C_MIN_BASELINE_ELIGIBLE_PER_SEED:
            sufficient = False
    ...
    if not sufficient or len(rates) != len(PHASE_C_MASTER_SEEDS):
    ...
    return {
        "stage": "warning-blind frontier reproducibility audit Phase C",
...
def write_phase_c_cell(upstream_checkout: str | Path, cell_index: int, output: str | Path) -> Path:
    destination = Path(output)
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        json.dumps(run_phase_c_cell(upstream_checkout, cell_index), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return destination
