"""Per-seed runner and aggregator for high-precision Phase-O frontier validation."""
...
from .frontier_precision_phase_o import (
    PHASE_O_MIN_BASELINE_ELIGIBLE_PER_SEED,
    PHASE_O_PREFIX_REPLICATES,
    PHASE_O_REPLICATES_PER_SEED,
    expected_prefix,
    phase_o_manifest,
)
from .frontier_refinement_manifest import PHASE_D_MASTER_SEEDS, phase_d_cells
...
def run_phase_o_seed(upstream_checkout:str|Path, master_seed:int)->dict[str,Any]:
    if master_seed not in PHASE_D_MASTER_SEEDS: raise ValueError("not a locked Phase-D seed")
    ...
    with _upstream_import_path(checkout):
        ...
        for phase_cell in phase_d_cells():
            ...
            spec=replace(experiments.standard_profile(),experiment_id=f"frontier_precision_phase_o_{phase_cell.cell_index:02d}",generations=1,replicates=PHASE_O_REPLICATES_PER_SEED,master_seed=master_seed,area_reference_values=(anchor.area_reference,),interaction_feedback_values=(anchor.interaction_kappa,),interaction_barrier_values=(0.5,))
            with patched_protocol002_mutation_runner(mutation,coordinate):
                ...
                if len(cells)!=1: raise RuntimeError("Phase O requires exactly one H1 cell per p_star and master seed")
                cell=cells[0]; isolated=chain._scenario_map(spec)[experiments.SCENARIO_EQUAL_ISOLATED]
                for record in cell.replicates:
                    base={"master_seed":master_seed,"replicate":record.replicate_index,"p_star":coordinate.p_star,"kappa_mu":coordinate.kappa_mu,"calibration_seed":record.seed}
                    ...
                    template=chain.parameters_for_cell(spec,isolated,replace(cell.parameters,interaction_barrier=anchor_barrier),seed=record.seed)
    ...
    for phase_cell in phase_d_cells():
        p=float(phase_cell.coordinate.p_star); rows=[r for r in attempts if r["p_star"]==p]; prefix=[r for r in rows if r["replicate"]<PHASE_O_PREFIX_REPLICATES]; pe=[r for r in prefix if r["eligible_for_trait_loss_denominator"]]; pl=[r for r in pe if r["trait_loss_observed_post_baseline"] is True]
        ee,el=expected_prefix(master_seed,p); ok=len(pe)==ee and len(pl)==el; prefix_ok &= ok; eligible=[r for r in rows if r["eligible_for_trait_loss_denominator"]]; losses=[r for r in eligible if r["trait_loss_observed_post_baseline"] is True]
        summaries.append({"p_star":p,"baseline_eligible":len(eligible),"trait_loss":len(losses),"trait_loss_rate":None if not eligible else len(losses)/len(eligible),"precision_sufficient":len(eligible)>=PHASE_O_MIN_BASELINE_ELIGIBLE_PER_SEED,"prefix":{"observed_eligible":len(pe),"observed_losses":len(pl),"expected_eligible":ee,"expected_losses":el,"matches_historical":ok}})
    return {"stage":"frontier precision validation Phase O","master_seed":master_seed,"prefix_audit_passed":prefix_ok,"condition_summaries":summaries}


def aggregate_phase_o(payloads:Iterable[dict[str,Any]])->dict[str,Any]:
    payloads=tuple(payloads)
    if sorted(p["master_seed"] for p in payloads)!=sorted(PHASE_D_MASTER_SEEDS): raise RuntimeError("requires five locked Phase-D seeds")
    ...
    for phase_cell in phase_d_cells():
        ...
        sufficient=all(n>=PHASE_O_MIN_BASELINE_ELIGIBLE_PER_SEED for _,n in blocks); rates=tuple(k/n for k,n in blocks); regime="insufficient_precision" if not sufficient else _regime(rates); pooled=sum(k for k,_ in blocks)/sum(n for _,n in blocks); stat,df,eqp=pearson_equal_rate_test(tuple(blocks)); ref=ensemble_gate_pass_probability(tuple(n for _,n in blocks),pooled); regimes[f"{p:.3f}"]=regime
        summaries.append({"p_star":p,"blocks":[{"master_seed":s,"losses":k,"eligible":n,"rate":k/n} for s,(k,n) in zip(PHASE_D_MASTER_SEEDS,blocks,strict=True)],"pooled_loss_rate":pooled,"historical_gate_regime_at_full_precision":regime,"pearson_equal_rate_p_value":eqp,"homogeneous_reference_gate_fail_probability":1-ref})
    central=regimes["0.350"]; neighbours=(regimes["0.325"],regimes["0.375"])
    ...
    elif central!="R4_highrep": decision="central_r4_not_reproduced"
    elif any(v!="R4_highrep" for v in neighbours): decision="at_least_one_immediate_neighbour_outside_r4_at_high_precision"
    else: decision="historical_narrow_r4_frontier_disappears_at_high_precision"
    return {"stage":"frontier precision validation Phase O","manifest":phase_o_manifest(),"prefix_audit_passed":prefix_ok,"decision":decision,"regime_by_p_star":regimes,"p_star_summaries":summaries,"per_seed_payloads":list(payloads)}


def load_and_aggregate_phase_o(paths:Iterable[str|Path])->dict[str,Any]:
    return aggregate_phase_o([json.loads(Path(p).read_text()) for p in paths])
