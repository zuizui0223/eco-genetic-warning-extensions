...
### S2.1 Post-review fixed-area fragmentation-gradient sensitivity
...
The binary audit did not retrofit a continuous score to the locked endpoint family. A subsequent and explicitly exploratory protocol was instead committed before any continuous AUC was calculated. It fixed `E_H(t) = 1 - H(t)/H(0)` for `H_alpha` and `H_gamma` at all three existing landmarks. At each landmark, trajectories lost at or before that generation were excluded; cases had loss in `(t,120]` and controls remained event-free through 120. AUC was the Mann–Whitney concordance probability with half credit for ties. Two-sided 95% percentile intervals used 10,000 stratified trajectory-level bootstrap samples, resampling cases and controls separately with base RNG seed `20260826`. No slope, alternative score, selected landmark, endpoint pooling or p-value was opened. The six cells within an ensemble reuse trajectories and are not independent replicates.

### S8.2 Post-lock Eschscholzia descriptive metadata reconstruction

After the primary `multi_endpoint_not_identifiable` decision and the one-key typo-sensitivity STOP were permanent, a third path was separately preregistered for descriptive information recovery only. It permitted the exact literal Habitat corrections at arrays `1||3` and `1||4` and required the observed mismatch set to equal those two keys. Any other mismatch or remaining mismatch required a pre-model stop. If metadata passed, the unchanged F preparation, S0/S1/S2 ridge models, array-level validation and 10,000-bootstrap procedure were to be reused without row deletion or response repair. The metadata mismatch count became zero, but the unchanged preparation gate rejected an invalid primary response before modelling.
