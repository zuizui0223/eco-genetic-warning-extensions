...
Ecological forecasts can fail when measured summaries erase future-relevant structure. In a finite multipatch eco-genetic model, two states with identical census, interaction, allele-frequency and trait marginals, `H_alpha`, `H_gamma` and `F_ST` but opposite cross-layer alignment differed in their exact next interaction transition by 0.2543. A separately prospectively locked propagation experiment then followed 1,500 paired trajectories under one fixed deterioration path. Anti-aligned minus aligned functional-loss risk was 0.0 percentage points at generation 5, +0.33 at 10, +5.33 (95% CI +2.04 to +8.62) at 20 and +5.20 (+1.96 to +8.44) at 40. Thus matching marginal summaries did not imply dynamic equivalence, and the predictive relevance of hidden state structure was horizon-dependent. Operator-specific tests further showed that a scalar connectivity label did not transport across distinct movement closures. These results support target-, horizon- and operator-dependent state representation, not a universal timescale or complete-state rule.
