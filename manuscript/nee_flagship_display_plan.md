# NEE flagship display plan — four-operator mechanism

## Principle

The main figures carry only exact mathematics and locked finite-model evidence. Natural systems remain Introduction/Discussion projections and receive no main-data panel.

## Figure 1 — From fragmentation to causal pathway balance

Define the manuscript in one view:

- fragmentation separates persistence from functional support;
- matched conventional marginals can hide transition-relevant cross-layer covariance;
- four life-cycle roles determine the later route:
  1. **sorting** — local q-dependent allele selection;
  2. **trait–allele buffering** — allele-linked recruitment;
  3. **interaction–bundle recoupling** — direct T/G -> q support feedback;
  4. **failure gate** — density -> q feedback;
- early marginal erosion need not discriminate which route is dominating.

Central synthesis:

`cross-layer covariance -> sorting <-> repair (buffering + recoupling) -> density-feedback collapse gate`

Footer: landscape labels, alignment scores and the model-specific algebraic weights are not universal mechanisms.

## Figure 2 — Fragmentation separates functional support from persistence

Source: pinned EGC fixed-area fragmentation gradient.

Panel A:
- one patch potential high-trait viability: 1,037/1,037;
- every tested subdivision: 0/1,037;
- realised high-trait occupancy remains approximately 99.6–100% at generation 30.

Panel B: retained interaction, local effective size and realised high-trait mass across 1,2,3,4,6,8,12,16 patches.

Take-home: one structural perturbation does not define one biological deterioration coordinate.

## Figure 3 — Why matched marginals reach different futures

### Panel A — exact immediate divergence

- mean support identical: `0.68` versus `0.68`;
- support variance: `0.0245` versus `0.0005`, **49×**;
- maximum exact generation-1 q difference: **0.2543**;
- show the support-variance covariance identity.

Take-home: covariance changes where support is concentrated at fixed marginals.

### Panel B — sorting operator

High-allele selection:

`logit(p+) - logit(p) = log(0.75 + 0.4q)`.

Show:
- `dp+/dq > 0`;
- exact switch `q*=0.625`, equal to the declared high-trait viability threshold;
- one selection step increases `Cov(q,logit p)` whenever q varies spatially;
- focused 6,000-pair g40 proof:
  - baseline `RR-AA = +6.65 pp` `[+5.07,+8.23]`;
  - delete only q -> allele selection `RR-AA = -0.23 pp` `[-1.80,+1.34]`;
  - preregistered DID **+6.883 pp** `[+5.800,+7.967]`.

Take-home: q-dependent allele sorting is an exact operator and a resolved single-edge causal contributor.

### Panel C — two repair operators

#### C1 Recruitment buffering

For resident high-trait mass `m` and high-allele frequency `p`:

`r=(m+p)/2` under locked `h=.5`.

Thus `|r-p|=.5|m-p|`: trait–allele mismatch contracts **50%** before selection.

Locked deletion:
- g20 DID **-9.00 pp** `[-13.29,-4.71]`;
- g40 DID **-8.33 pp** `[-12.53,-4.14]`.

#### C2 Direct feedback recoupling

Define `B=.75T+.25G` and `S=.6q+.4B`.

Thus `|S-B|=.6|q-B|`: interaction–bundle mismatch contracts **40%** at the support stage.

Relative to q-only:

`logit(q_full+) - logit(q_qonly+) = 1.8 d (B-q)` under the locked weights.

Secondary paired contrasts from the prospectively locked six-condition experiment:
- g20 RR direct-feedback benefit **+8.53 pp** `[+5.21,+11.85]`, RR-minus-AA benefit **+7.93 pp** `[+3.29,+12.58]`;
- g40 RR benefit **+7.80 pp** `[+4.79,+10.81]`, benefit difference **+6.33 pp** `[+1.85,+10.82]`.

Take-home: recruitment repairs trait–allele mismatch; direct feedback repairs interaction–bundle mismatch and preferentially buffers the reversed configuration in the locked ensemble.

### Panel D — density-feedback failure gate

Exact q update:

`q+ = sigmoid[4.5(dq-theta)]`.

Show:
- below carrying capacity, `dq+/dN > 0`;
- smooth positive loop `q down -> N down -> density down -> q down`;
- exact headroom boundary for retaining `q+ >= .625`:

`d*q >= theta + 0.1135168053`;

- required `d*q`: **0.6160** at g1, **0.6635** at g20, **0.7135** at g40;
- deleting density -> q:
  - g20: 0/1,500 loss in both AA and RR; paired reductions **38.33 pp** and **42.53 pp**;
  - g40: reductions **57.47 pp** and **59.60 pp**.

Take-home: density feedback does not create the directional sorting advantage; it opens and amplifies the collapse regime.

### Figure-3 bottom-line synthesis

`covariance creates divergence -> q-dependent selection sorts -> recruitment/direct feedback repair mismatch -> density feedback determines whether deterioration becomes self-amplifying`

## Figure 4 — A perfectly early marginal marker can fail to distinguish fate

Source: frozen warning-validity audit.

Panel A: inherited denominator — event leads 35/35; non-event firing 48/48.

Panel B: fresh denominator — event leads 33/33; non-event firing 49/49.

Panel C: exact identity:
- perfect precedence -> sensitivity = 1;
- specificity is not fixed by event-only ordering;
- binary `AUC=(1+specificity)/2`;
- observed specificity=0 -> AUC=0.5.

Take-home: detecting marginal erosion is not the same as identifying whether sorting, repair or collapse feedback controls fate.

## Extended Data / Supplementary information

1. Canonical interaction-map theorem and barrier interval.
2. Full fragmentation-gradient paired summaries.
3. Complete matched-marginal transition certificate.
4. Original propagation precision grid and non-universal-direction boundary.
5. Six-condition relational mechanism decomposition.
6. Eight-intervention pathway decomposition and mediator trajectories.
7. Focused 6,000-pair allele-sorting proof and operator theorem.
8. Exact recruitment-buffer theorem and parent-code certificate.
9. Exact direct-feedback recoupling theorem and locked paired contrast derivation.
10. Exact density-feedback headroom theorem and locked paired risk reductions.
11. All six warning endpoints and full-denominator metrics.
12. Evidence-role map and literature-based ecological projection table: limited buffering, movement recoupling, temporal memory and coordinated deterioration.
