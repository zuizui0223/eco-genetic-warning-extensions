...
The empirical counterpart of state sufficiency is conditional predictive redundancy of the upstream fragmentation descriptor after a measured process state is supplied. Natural datasets are not pooled; each is evaluated at its own ecological resolution with whole ecological units held out.
...
These are **ecological partial-state tests**, not complete eco-genetic convergence tests: neither system synchronizes `D/I/T/C/R/G_by_cohort/M/A` in full.
...
These are not demonstrations that geography or urbanisation is irrelevant. Nor do they prove that the tested interaction states are complete. Their common result is narrower: **an upstream fragmentation descriptor can have ecological effects yet fail to add transferable prediction after a more proximal measured process state is supplied**.
...
The parent and extension repositories are separate computational provenance units. The extension pins the parent scientific state at `dd8ee379d0d3518194c767d16402042525bc00dc`. Machine-readable evidence, workflow/artifact provenance, condition ledgers, state-representation audits, natural-data preregistrations and submission-build instructions are version controlled. Third-party natural raw data are not committed; the analyses lock their public DOI/version provenance and retain compact derived summaries. Historical low-replicate and allele-mixing results are retained unchanged; precision replays, process-resolved closures, fresh warning replication, cross-layer alignment tests and natural residual-origin tests are additive evidence layers and do not overwrite historical provenance.
